...
func main() {
	...
	docketMetrics := repository.NewDocketMetricsRepository(db)
	docketStatus := repository.NewDocketStatusRepositoryService(DB)
	...
	go worker.StartDocketStatusResultSubscriber(docketMetrics, docketStatus)
